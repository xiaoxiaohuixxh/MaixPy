...
STATIC mp_obj_t machine_help(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
	...
	if(args[ARG_func].u_int == 204)
	{
		...
		printf("All function description\n");
		printf ("+-------------------+----------------------------------+\n") ;
		printf ("|     Function      |            Description           |\n") ;
		printf ("+-------------------+----------------------------------+\n") ;
		for(int i = 0;i < ALL_FPIOA__FUNC;i++)
		{    
			/*malloc memory*/
...
STATIC MP_DEFINE_CONST_FUN_OBJ_KW(machine_help_obj, 0,machine_help);
...
STATIC const mp_rom_map_elem_t pyb_fpioa_locals_dict_table[] = {
    ...
    { MP_ROM_QSTR(MP_QSTR_help), MP_ROM_PTR(&machine_help_obj) },
